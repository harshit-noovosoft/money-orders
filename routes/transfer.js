...
router.post('/',async (req,res)=>{
    ...
    const fromUserId = await client.query('select user_id from users where user_name = $1',[fromUsername]);
    const toUserId = await client.query('select user_id from users where user_name = $1',[toUsername]);
    ...
    const entryToDB = await client.query('Insert into transactions (transaction_id, transaction_type, amount, from_user_id, to_user_id) values ($1,$2,$3,$4,$5)',
        [transactionId,'Transfer',amount,fromUserId.rows[0]['user_id'],toUserId.rows[0]['user_id']]);
